...
abstract class FacultiesDAO(db: MySQLProfile.backend.Database)(implicit ec: ExecutionContext) extends TableQuery(new Faculties(_)) {
  def findById(id: Int): Future[Option[FacultyRecord]] =
    db.run(this.filter(_.id === id).result).map(_.headOption)


  def store(faculty: FacultyRecord): Future[FacultyRecord] =
    db.run(this returning this.map(_.id) into ((acc, id) => acc.copy(id = id)) += faculty)
