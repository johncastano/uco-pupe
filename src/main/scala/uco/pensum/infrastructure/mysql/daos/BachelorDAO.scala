...
class Bachelors(tag: Tag) extends Table[BachelorRecord](tag,"BACHELORS"){
  ...
  def faculties = foreignKey("FACULTY_ID",facultyId, tables.faculties)(_.id, onUpdate=ForeignKeyAction.Restrict, onDelete=ForeignKeyAction.Cascade)

  def * = (id,facultyId, bachelorName) <> (BachelorRecord.tupled, BachelorRecord.unapply)
}

abstract class BachelorsDAO(db: MySQLProfile.backend.Database)(implicit ec: ExecutionContext) extends TableQuery(new Bachelors(_)) {
