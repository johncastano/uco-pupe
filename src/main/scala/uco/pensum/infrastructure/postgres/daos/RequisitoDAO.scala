...
abstract class RequisitosDAO(db: PostgresProfile.backend.Database)(
  ...
  def buscarPorCodigoPR(codigoPR: String): Task[List[RequisitoRecord]] =
    Task.deferFuture(
      db.run(this.filter(_.codigoAsignaturaPR === codigoPR).result)
        .map(_.toList)
    )
  ...
  def eliminarPorCodigoAsignatura(codigo: String): Task[Int] = Task.deferFuture(
    db.run(this.filter(_.codigoAsignatura === codigo).delete)
  )
